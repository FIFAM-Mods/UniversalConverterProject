...
EAGMoney *METHOD OnPlayerStartingConditionsLoan_GetMV(CDBPlayer *player, DUMMY_ARG, EAGMoney *out, CDBEmployee *employee) {
    *out = EAGMoney::EAGMoney(100, CURRENCY_EUR);
    return out;
}

void METHOD OnPlayerStartingConditions_FinishApply(FmVec<StartingCondition> &conditions) {
    for (UInt i = 0; i < conditions.size(); i++) {
        auto &condition = conditions[i];
        if (condition.m_nType == STARTINGCONDITION_LOAN) {
            CDBPlayer *player = GetPlayer(condition.m_nPlayerId);
            if (player) {
                void *contractOptions = CallMethodAndReturn<void *, 0xF98990>(player);
                EAGMoney buyOptionValue;
                CallMethod<0x11642D0>(contractOptions, &buyOptionValue);
                if (buyOptionValue.GetValueInCurrency(CURRENCY_EUR) == 100) {
                    player->UpdateMarketValue();
                    EAGMoney newBuyOption = player->GetMarketValue();
                    newBuyOption *= 0.8f + CRandom::GetRandomFloat(0.3f);
                    CallMethod<0x1164290>(contractOptions, newBuyOption);
                }
            }
        }
    }
    CallMethod<0x137C430>(&conditions);
}

EAGMoney *OnUpdateMVMultiplyByPrestigeFactor(EAGMoney *out, Double a, EAGMoney *b) {
    if (a == 0.0)
        *out = *b;
    else
        Call<0x0149DB17>(out, a, b);
    return out;
}
...
void PatchEABFFixes(FM::Version v) {
    if (v.id() == ID_FM_13_1030_RLD) {
        ...
        // fix: autogenerated buy option for loaned players
        patch::RedirectCall(0x137B364, OnPlayerStartingConditionsLoan_GetMV); // CPlayerStartingConditions::MakeLoan

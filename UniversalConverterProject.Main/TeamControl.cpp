...
String RoleName(UInt r) {
    static WideChar *roleNames[] = {
        L"ASSISTANT_COACH1",
        L"ASSISTANT_COACH2",
        L"AMATEUR_COACH",
        L"YOUTH_COACH",
        ...
        L"GENERAL_MANAGER",
        L"MARKETING_MANAGER",
        L"CONSTRUCTION_MANAGER",
        L"SPORTS_DIRECTOR",
        L"FAN_REPRESENTATIVE",
        L"SPOKESPERSON",
        L"LAWYER",
        L"GENERAL_SCOUT1",
        L"GENERAL_SCOUT2",
    ...
    String result = Utils::Format(L"%d", r);
    if (r >= 6 && r <= 30)
        result += Utils::Format(L" (%s)", roleNames[r - 6]);
    return result;
};

void DumpTeamStaff(Path const &teamStaffFolder, CDBTeam *team) {
    static WideChar *departmentNames[] = {
        L"COACHES",
        L"MEDICINE",
        L"MANAGERS",
        L"PR",
    ...
    if (team->GetTeamID().index == 0xFFFF)
        return;
    CTeamStaff *teamStaff = team->GetTeamStaff();
    if (!teamStaff)
        return;
    if (teamStaff->structure.departments[0]->roles.size() <= 6 && teamStaff->structure.departments[2]->roles.size() <= 4)
        return;
    Path l = teamStaffFolder / Utils::Format(L"%s.txt", TeamTag(team));
    SafeLog::WriteToFile(l, TeamTag(teamStaff->teamId));
    SafeLog::WriteToFile(l, L"\nDepartments:\n");
    for (UInt i = 0; i < 5; i++) {
        auto dep = teamStaff->structure.departments[i];
        SafeLog::WriteToFile(l, Utils::Format(L"%d (%s) - id %d type %d", i, departmentNames[i], dep->id, dep->type));
        Vector<String> roleNames;
        for (auto &k : dep->roles)
            roleNames.push_back(RoleName(k));
        SafeLog::WriteToFile(l, Utils::Format(L"Roles: %s\n", Utils::Join(roleNames, L", ")));
    }
    SafeLog::WriteToFile(l, L"Workers:\n");
    for (auto &[roleId, workerId] : teamStaff->roleToWorkerId) {
        CDBStaff *staff = GetStaff(workerId);
        String staffName = staff ? staff->GetName() : L"n/a";
        SafeLog::WriteToFile(l, Utils::Format(L"%s - %s (%d)", RoleName(roleId), staffName, workerId));
    }
}

void DumpTeamFanshops(CDBTeam *team) {
    auto fanshops = team->GetFanShops();
    for (UInt i = 0; i < fanshops->GetNumFanShops(); i++) {
        auto f = fanshops->GetFanShop(i);
        SafeLog::WriteToFile(L"dump_fanshops.txt", Utils::Format(L"%d\t%s\t%d",
            f->abroadID, CountryName(f->countryId), f->roleID), L"ID\tCountry\tRoleID");
    }
}

void DumpYouthCamps() {
    WideChar *positionNames[] = {
        L"MANAGER",
        L"CHIEF_EXEC",
        L"PRESIDENT",
        L"ASSISTANT_COACH",
    ...
    Path teamStaffFolder = "dump_team_staff";
    create_directories(teamStaffFolder);

    auto roleFactory = GetRoleFactory();
    for (auto &[id, r] : roleFactory->roles) {
        String strPos = Utils::Format(L"%d", r->positionId);
        if (r->positionId >= 1 && r->positionId <= 3)
            strPos += Utils::Format(L" (%s)", positionNames[r->positionId - 1]);
        else if (r->positionId >= 6 && r->positionId <= 25)
            strPos += Utils::Format(L" (%s)", positionNames[r->positionId - 3]);
        String city = Utils::Format(L"%d (%s)", r->cityId, GetCityName(r->cityId) ? GetCityName(r->cityId) : L"n/a");
        SafeLog::WriteToFile(L"dump_roles.txt", Utils::Format(L"%s\t%s\t%s\t%d\t%d\t%s",
            RoleName(r->id), strPos, r->name.c_str(), r->importance, r->specialType, city), L"ID\tPosition\tName\tImportance\tType\tCity");
    }

    for (UInt i = 1; i < 208; i++) {
        FmVec<CDBYouthcamp> *vec = &GetYouthcampList()->countries[i];
        for (UInt c = 0; c < vec->size(); c++) {
            CDBYouthcamp *y = &vec->begin[c];
            String city = Utils::Format(L"%d (%s)", y->cityId, GetCityName(y->cityId) ? GetCityName(y->cityId) : L"n/a");
            SafeLog::WriteToFile(L"dump_youth_camps.txt", Utils::Format(L"%d\t%s\t%s\t%d\t%d\t%s\t%d\t%s",
                y->id, CountryName(y->countryId), city, y->level1, y->level2, TeamTag(y->teamID), y->youthCoachRoleId, y->date.ToStr()),
                L"ID\tCountry\tCity\tLevel1\tLevel2\tTeam\tRoleID\tDate");
        }
    }

    DumpTeamStaff(teamStaffFolder, GetTeamByUniqueID(0x00291004));

    auto it = NetComStorageBegin(STORAGE_TEAMS);
    auto end = NetComStorageEnd(STORAGE_TEAMS);
    while (it.blockId != end.blockId || it.index != end.index) {
        CDBTeam *team = (CDBTeam *)it.object;
        if (team) {
            DumpTeamFanshops(team);
            DumpTeamStaff(teamStaffFolder, team);
        }

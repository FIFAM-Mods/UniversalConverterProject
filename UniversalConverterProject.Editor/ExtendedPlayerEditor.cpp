...
Bool METHOD OnWritePlayerFoomIdToDatabase(void *t, DUMMY_ARG, Char const *name) {
    PlayerExtension *ext = raw_ptr<PlayerExtension>(gDatabasePlayer, DEF_PLAYER_SZ);
...
void PatchExtendedPlayer(FM::Version v) {
    if (v.id() == VERSION_ED_13) {
        ...
        patch::RedirectCall(0x51FEF3, OnReadPlayerFoomIdFromDatabase);
        patch::RedirectCall(0x51B85D, OnWritePlayerFoomIdToDatabase);

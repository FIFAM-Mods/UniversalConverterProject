...
void METHOD SetupLowestLeagues_11(int _this) {
    if (*(DWORD *)(_this + 14284) && *(DWORD *)(_this + 14280)) {
        HWND *wnd = (HWND *)(_this + 3640);
        for (int i = 0; i < 3; i++) {
            SendMessageW(*wnd, 331, 0, 0);
            wchar_t *v4 = CallMethodAndReturn<wchar_t *, 0x530310>((void *)_this, "Common.NotAvailable", 0);
            LRESULT v5 = SendMessageW(*wnd, 323, 0, (LPARAM)v4);
            if (v5 != -1)
                SendMessageW(*wnd, 337, v5, 0);
            wnd += 20;
        ...
        int numLevels = CallMethodAndReturn<int, 0x4B2F90>(*(CCountry **)(_this + 14280), leagueIndices, numLeaguesOnLevels);
        if (numLevels > 0) {
            for (int i = 0; i < numLevels; i++) {
                if (numLeaguesOnLevels[i] > 1) {
                    for (int l = 0; l < numLeaguesOnLevels[i]; l++) {
                        CDBCompetition *comp = CallMethodAndReturn<CDBCompetition *, 0x4B2F10>(*(CCountry **)(_this + 14280), i, l);
                        if (comp) {
                            int comboBox = _this + 3612;
                            for (int c = 0; c < 3; c++) {
                                unsigned int compId = CallMethodAndReturn<unsigned int, 0x4CAD30>(comp);
                                const wchar_t *compName = CallMethodAndReturn<const wchar_t *, 0x4CD200>(comp);
                                CallMethod<0x40FC60>(0, comboBox, compName, compId);
                                comboBox += 80;
                            }
                        }
                    }
        ...
        int v14 = CallMethodAndReturn<int, 0x4A2CC0>(*(CClub **)(_this + 14284), 0);
        CallMethod<0x40FCA0>(0, (CComboBox *)(_this + 3612), v14);
        int v15 = CallMethodAndReturn<int, 0x4A2CC0>(*(CClub **)(_this + 14284), 1);
        CallMethod<0x40FCA0>(0, (CComboBox *)(_this + 3692), v15);
        int v16 = CallMethodAndReturn<int, 0x4A2CC0>(*(CClub **)(_this + 14284), 2);
        CallMethod<0x40FCA0>(0, (CComboBox *)(_this + 3772), v16);
    }
}
...
void PatchLowestLeagues(FM::Version v) {
    if (v.id() == ID_ED_13_1000) {
        ...
        patch::RedirectJump(0x422F00, SetupLowestLeagues_11);
    }
}
